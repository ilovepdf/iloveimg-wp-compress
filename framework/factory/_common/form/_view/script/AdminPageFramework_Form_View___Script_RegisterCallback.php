...
class iLoveIMGAdminPageFramework_Form_View___Script_RegisterCallback extends iLoveIMGAdminPageFramework_Form_View___Script_Base {
    static public function getScript() {
        return <<<JAVASCRIPTS
(function ( $ ) {
            
    // Callback containers.
    $.fn.aiLoveIMGAdminPageFrameworkAddRepeatableFieldCallbacks        = [];            
    $.fn.aiLoveIMGAdminPageFrameworkRepeatFieldCallbacks               = [];    // 3.8.8+
    $.fn.aiLoveIMGAdminPageFrameworkRemoveRepeatableFieldCallbacks     = [];
    $.fn.aiLoveIMGAdminPageFrameworkSortedFieldsCallbacks              = [];            
    $.fn.aiLoveIMGAdminPageFrameworkStoppedSortingFieldsCallbacks      = [];
    $.fn.aiLoveIMGAdminPageFrameworkAddedWidgetCallbacks               = [];
    $.fn.aiLoveIMGAdminPageFrameworkStoppedSortingSectionsCallbacks    = [];    // 3.8.0+
    
    /**
     * Gets triggered when the + (add) button of a repeatable field is pressed.
     */  
    $( document ).bind( 'iloveimg_added_repeatable_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){          
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aiLoveIMGAdminPageFrameworkAddRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            ...
            if ( -1 === $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {                
                console.warn( 'Admin Page Framework (' + sFieldType + ' field type): The `added_repeatable_field` callback argument for the `registeriLoveIMGAdminPageFrameworkCallbacks` method is deprecated. Use `repeated_field` instead.' );
            }
     ...
     * 
     * Unlike the `iloveimg_added_repeatable_field` event, this does not call the callback function
     ...
     */
    $( document ).bind( 'iloveimg_repeated_field', function( oEvent, iCallType, oModelContainer ){
        ...
        $.each( $.fn.aiLoveIMGAdminPageFrameworkRepeatFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( -1 !== $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                return true;    // continue
     ...
     * @since       3.8.0
     */
    $( document ).bind( 'iloveimg_stopped_sorting_sections', function( oEvent ){  

        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aiLoveIMGAdminPageFrameworkStoppedSortingSectionsCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
     ...
     */
    /* $( document ).bind( 'iloveimg_removed_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aiLoveIMGAdminPageFrameworkRemoveRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];       
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
    ...
    $.fn.callBackSortedFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.aiLoveIMGAdminPageFrameworkSortedFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
    ...
    $.fn.callBackStoppedSortingFields = function( sFieldType, sID, iCallType ) {
        var oThisNode = this;
        $.fn.aiLoveIMGAdminPageFrameworkStoppedSortingFieldsCallbacks.forEach( function( aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( 2 < _aFieldTypes.length && -1 === $.inArray( sFieldType, _aFieldTypes ) ) {
                return true; // continue
    ...
    $( document ).bind( 'iloveimg_saved_widget', function( event, oWidget ){
        $.each( $.fn.aiLoveIMGAdminPageFrameworkAddedWidgetCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ];
            if ( ! $.isFunction( _hfCallback ) ) { 
                return true;    // continue
     ...
     */
    $.fn.registeriLoveIMGAdminPageFrameworkCallbacks = function( oCallbacks, aFieldTypeSlugs ) {
        ...
        $.fn.aiLoveIMGAdminPageFrameworkAddRepeatableFieldCallbacks.push( 
            [ oCallbacks.added_repeatable_field, aFieldTypeSlugs ]
        );

        $.fn.aiLoveIMGAdminPageFrameworkRepeatFieldCallbacks.push(  // 3.8.8+
            [ oCallbacks.repeated_field, aFieldTypeSlugs ]
        );        
        $.fn.aiLoveIMGAdminPageFrameworkRemoveRepeatableFieldCallbacks.push( 
            [ oCallbacks.removed_repeatable_field, aFieldTypeSlugs ]
        );
        $.fn.aiLoveIMGAdminPageFrameworkSortedFieldsCallbacks.push( 
            [ oCallbacks.sorted_fields, aFieldTypeSlugs ]
        );
        $.fn.aiLoveIMGAdminPageFrameworkStoppedSortingFieldsCallbacks.push( 
            [ oCallbacks.stopped_sorting_fields, aFieldTypeSlugs ]
        );
        $.fn.aiLoveIMGAdminPageFrameworkAddedWidgetCallbacks.push( 
            [ oCallbacks.saved_widget, aFieldTypeSlugs ]
        );

        // 3.8.0
        $.fn.aiLoveIMGAdminPageFrameworkStoppedSortingSectionsCallbacks.push( 
            [ oCallbacks.stopped_sorting_sections, aFieldTypeSlugs ]
        );        

    };
    /**
     * An alias of the `registeriLoveIMGAdminPageFrameworkCalbacks()` method.
    ...
    $.fn.registerAPFCallback = function( oCallbacks, aFieldTypeSlugs ) {
        $.fn.registeriLoveIMGAdminPageFrameworkCallbacks( oCallbacks, aFieldTypeSlugs );
    }
